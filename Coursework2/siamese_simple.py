...
class Siamese(object):
    ...
    def __init__(self):
        # Initialize
        # First input image
        self.tf_input_1 = tf.placeholder(tf.float32, [None, 2048], name = 'input_1')
        # Second input image
        self.tf_input_2 = tf.placeholder(tf.float32, [None, 2048], name = 'input_2')
    ...
    def network(self, tf_input):
        # Setup FNN
        fc1 = self.fc_layer(tf_input = tf_input, n_hidden_units = 4096, variable_name = 'fc1')
        ac1 = tf.nn.relu(fc1)
        fc2 = self.fc_layer(tf_input = ac1, n_hidden_units = 1024, variable_name = 'fc2')
        ac2 = tf.nn.relu(fc2)
        fc3 = self.fc_layer(tf_input = ac2, n_hidden_units = 256 , variable_name = 'fc3')
        return fc3
    ...
    def loss_contrastive(self, margin = 8.0):
...
#        assert os.path.exists(MODEL_DIR + MODEL_NAME)
        self.saver.restore(self.sess, MODEL_DIR + MODEL_NAME+episode)
    ...
    def save_model(self, episode):
        ...
        self.saver.save(self.sess, MODEL_DIR + MODEL_NAME + episode)        
